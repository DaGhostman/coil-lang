...
4. **Lower** — after multi-file link, `finalize_bytecode` runs IL opts (`jump_thread`, JMP-only `dead_block`, `stack_dce`) then `il::lower`: fuse-select with **label barriers**, assign PCs once, encode `Vec<Byte>`. Enabled fuse patterns: const fold, `BinSlotSlot`, `CmpJmpf` / `LogNotJmpf` / `BinSlotImmJmpf`. Still deferred: `BinSlotImm` and `*Return` (attr-inline / JMP-to-RETURN joins). CALL/TailCall/MakeCoro/CodePtr/MakePolyFn use `IlOp::Entry`; production abs JMP is rejected in lower — no post-lower `peephole::fuse_bytecode` / `adjust_target` hot path.
