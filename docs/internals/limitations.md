...
**Local slot promotion (first slice).** `opt/slot_promote.rs` extends the tell-safe model across CFG blocks with slot **alias** forwarding (`LOAD a; STORE b` rewrites later `LOAD` / `BinSlot*` uses of `b` to `a`), same-def joins when forward preds agree, and loop-invariant aliases into a header when the natural loop does not store the bound slot or its deps. Unused `LOAD a; STORE b` copies are elided when tell allows or a straight-line higher store (earlier or later) dominates the cursor floor. **Store-destination coalescing** rewrites `STORE t; …; LOAD t; STORE s` to write `s` when per-block slot liveness shows `t`/`s` do not interfere on the coalesced span and tell still covers the floor (`s >= t` or the usual remove-store proof); overlapping ranges such as mandelbrot `tr`/`zr` refuse. Const/ConstPool clones at LOAD sites stay with `copy_prop` (re-cloning after LICM breaks call-arg peel packing). Full SSA rename, general loop-carried promotion, and operand-stack retention across calls remain deferred (see the pass header).
