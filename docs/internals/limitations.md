...
**Local slot promotion (first slice).** `opt/slot_promote.rs` extends the tell-safe model across CFG blocks with slot **alias** forwarding (`LOAD a; STORE b` rewrites later `LOAD` / `BinSlot*` uses of `b` to `a`), same-def joins when forward preds agree, and loop-invariant aliases into a header when the natural loop does not store the bound slot or its deps. Unused `LOAD a; STORE b` copies are elided when tell allows or a later straight-line higher store dominates the cursor floor. Const/ConstPool clones at LOAD sites stay with `copy_prop` (re-cloning after LICM breaks call-arg peel packing). Full SSA rename, general loop-carried promotion, operand-stack retention across calls, and store-destination coalescing remain deferred (see the pass header).
