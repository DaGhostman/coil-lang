...
`STORE` (packed `n`) pops `n` then floors tell to `max(tell, max_written_slot + 1)` — height does not follow that floor. `Seek s` sets tell to `s`. `CALL` sets the callee frame base to `tell - arity`; the matching return seeks to that base and pushes, so the caller-relative tell effect is `-arity + 1`. Unifying the two would either make `sp` lie about height (break fuse/canon/convoy SP gates) or make `tell` ignore STORE floors (break slot_promote / dead_store). Split asserted in `sp.rs` / `tell.rs` (`store_does_not_raise_operand_height`, `store_floor_is_not_operand_height`, `store_floor_then_call_leaves_tell_above_sp`). `Call` uses `call_arity_delta` (`1 - arity`), not JumpIfMatch's `signed_arity_delta` (`arity - 1`).
