...
**Operand-order canon.** `il::canon` rewrites Known-SP `Const; Load; op` and high-then-low `Load; Load; op` windows into preferred forms before algebraic/fuse. It refuses float ops, `ConstPool`, residual `Byte`, Unknown SP, and non-commutative ops (`SUB`/`DIV`/`MOD`/`SHL`/`SHR`/`Pow`); ordered cmps flip polarity on swap (`LE`↔`GT`, `LEQ`↔`GEQ`). No float reassociation. Loop bounds' `i < bound` header match accepts both `Load i; Load b; LE` and the post-canon `Load b; Load i; GT` (and the matching `BinSlotSlot` forms).
