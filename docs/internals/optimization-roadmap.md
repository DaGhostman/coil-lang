...
| Cast spill → `FloatChainStore` | mandelbrot `float_chain_cast_blocked=1` (`cr`) | material in mandelbrot float body | **defer** (no opcode) | Tried IL `CastIntToFloat; STORE t; LOAD t` spill (`il::cast_spill`, default off): raises LOAD/STORE and residual float_arith without increasing `FloatChainStore`. Prefer a tighter codegen schedule for `cr` or accept the near-miss. |
