...
**GC finalizers** are different: an inherent `fn drop()` on a class runs when the instance is unmarked after a mark-sweep (`gc::collect()` or automatic GC), and again at VM exit for anything still allocated. Order is unspecified; `defer` stays the deterministic function-exit tool. See [`gc`](../../references/gc.md).
