...
| `prelude::ops::{Add,Eq,From,Into,…}` | Virtual module | Auto-imported operator / conversion traits |
...
| Category | Examples |
|----------|----------|
| Collections API | `sort`, lazy range iterators (`push` / `len` / `for-in` are builtins) |
