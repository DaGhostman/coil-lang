...
The same literal also coerces to **`[byte]` / `[byte; N]`** (full UTF-8 byte sequence) under an expected byte-array type or via `"hi" as [byte]` / `"hi" as [byte; 2]`. Fixed `[byte; N]` requires exactly `N` decoded bytes. This is a compile-time rewrite to `CONST` + `MakeArray` — not a runtime `Into` on arbitrary strings.
