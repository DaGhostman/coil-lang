...
- `codegen_var_types` is a side-table workaround for pre-walk / infer ID misalignment inside function bodies. **Partial progress:** `infer_function` / lambdas now assign Fragment + Argument NodeIds (matching codegen's `do_compile(args)`, still skipping type-annotation children) so free-fn / lambda body caches line up with `emit_idx`; Identifier codegen prefers the span cache before the name table. Remaining: Method/Function wrapper IDs when callers bypass `infer` (impl / typeclass methods), orphaned param type-annotation IDs, and retiring the side table for match bindings.
