...
- **Stack IL / lower.** Codegen emits `compiler/src/il` with symbolic labels; `finalize_bytecode` runs IL opts (`jump_thread` / `dead_block` / `stack_dce` / `return_convoy` / `bin_join_convoy` / `multi_op_join_convoy`) then lowers once (fuse-select + PC assign). Hot-path ops are typed `IlOp` variants lifted on absorb (`Load`/`Const`/`Bin`/`BinSlot*`/`*Return`/…); `IlOp::Byte` is the long-tail escape hatch. `return_convoy` sinks identical immediate join producers across a return-label cluster (`Label`+…+`RETURN`) into fused `*Return`. `bin_join_convoy` sinks identical plain binop tails to `BinReturn`, or identical `BinSlotImm`/`BinSlotSlot` to one copy before `RETURN` (Ord-safe: immediate producers only; refuse mixed/conditional preds). `multi_op_join_convoy` sinks identical length-2..=4 compute suffixes at return joins and unambiguous non-return continuations when SP-in agrees (`il::sp`); accepts `JMP`/`JMPF`/`JMPT` preds (jump-pred template when no fall-through); refuse Unknown heights / `JumpIfMatch` / jump-only joins. Label binds and absolute JMP targets inside a fuse window are barriers; `*Return` also refuses a stacked-value join on window[0]. Entry labels remap `functions` after opts; CALL/CodePtr absorb as `IlOp::Entry`. `IlFunc` metadata (name, entry, code span) is recorded on `CodeBuf`. Tiny inlining candidacy uses IL emitting ops (`code_slice_ops` / `is_tiny_inline_il`), may expand a sole `ConstReturnImm`/`LoadReturnSlot`/`BinReturn`, and remaps `BinSlotImm`/`BinSlotSlot` slots via caller temps. Nested VM returns from fused `ConstReturnImm` / `BinReturn` / `LoadReturnSlot` must call `capture_nested_return`. Soft CPU baseline: `./scripts/poop_baseline.sh`. `JumpIfMatch` diamond sinks remain deferred.
...
- **Match.** Threaded-code layout with per-outer-tag groups; multi-arm groups emit an inner `JUMP_IF_MATCH` test chain. `JumpIfMatch`: `operands[31:16]` = tag, `[15:0]` = pool index → absolute target. Nested record patterns use `UnpackAt` (slot-based), not top-of-stack `Unpack`.
