...
- **Stack IL / lower.** Codegen emits `compiler/src/il` with symbolic labels; `finalize_bytecode` runs IL opts (`jump_thread` / `dead_block` / `stack_dce` / `return_convoy`) then lowers once (fuse-select + PC assign). `return_convoy` sinks identical join producers into fused `*Return`. Label binds and absolute JMP targets inside a fuse window are barriers; `*Return` also refuses a stacked-value join on window[0]. Entry labels remap `functions` after opts; CALL/CodePtr absorb as `IlOp::Entry`. Tiny inlining candidacy uses IL emitting ops (`code_slice_ops` / `is_tiny_inline_il`). Nested VM returns from fused `ConstReturnImm` / `BinReturn` / `LoadReturnSlot` must call `capture_nested_return`.
