...
- **Stack IL / lower.** Codegen emits `compiler/src/il` with symbolic labels; `finalize_bytecode` lowers once (fuse-select + PC assign). Label binds are fusion barriers — no match `DUPLICATE; POP` barrier. Absolute CALL/CodePtr targets still bump on static-init splice via `CodeBuf::bump_absolute_entry_targets`. Nested VM returns from fused `ConstReturnImm` / `BinReturn` / `LoadReturnSlot` must call `capture_nested_return`.
