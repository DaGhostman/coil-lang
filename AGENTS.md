...
- **Stack IL / lower.** Codegen emits `compiler/src/il` with symbolic labels; `finalize_bytecode` runs IL opts then lowers once (fuse-select + PC assign). Label binds are fusion barriers. `BinSlotImm` and `*Return` fusions stay deferred (attr-inline / JMP-to-RETURN). Entry labels remap `functions` after opts; CALL/CodePtr absorb as `IlOp::Entry`. Nested VM returns from fused `ConstReturnImm` / `BinReturn` / `LoadReturnSlot` must call `capture_nested_return`.
