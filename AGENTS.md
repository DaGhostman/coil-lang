...
- **Stack IL / lower.** Codegen emits `compiler/src/il` with symbolic labels; `finalize_bytecode` runs IL opts then lowers once (fuse-select + PC assign). Label binds and absolute JMP targets inside a fuse window are barriers; `*Return` also refuses a join on window[0] (JMP-to-join would ignore the stacked arm value). Entry labels remap `functions` after opts; CALL/CodePtr absorb as `IlOp::Entry`. Nested VM returns from fused `ConstReturnImm` / `BinReturn` / `LoadReturnSlot` must call `capture_nested_return`.
