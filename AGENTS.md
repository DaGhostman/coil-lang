...
- Virtual `thread` module (`use thread::*`): `spawn`/`join`/`detach`, `channel`/`send`/`recv`/`close`, `mutex`/`with_lock`, `rwlock`/`with_read`/`with_write`; opaque `Thread`, `Sender`, `Receiver`, `Mutex`, `RwLock`; `thread::Error` sum type. One `Machine` per OS worker; `Pipeline::wire_thread_program` shares bytecode + function table. Host natives (`thread_spawn`, …) via `HostInvoke` — no new opcodes / no `ARCHIVE_VERSION` bump for threads. Nullary fns are sealed as `unit -> R` for `spawn(f)`; nested VM returns from fused `ConstReturnImm`/`BinReturn`/`LoadReturnSlot` must call `capture_nested_return` in `machine/src/vm.rs`.
- Virtual `regex` module (`use regex::*`): PCRE2 via `pcre2` crate / HostInvoke (`regex_compile`, …); opaque `Regex`; flags `i`/`m`/`s`/`x`/`u` via `RegexBuilder`; `find_all` / `captures_all` / `split` / `replace` — **no new opcodes** / no `ARCHIVE_VERSION` bump.
- Machine/compiler Cargo features `crypto`, `time`, and `regex` (all default-on) gate the virtual `crypto` / `time` / `regex` modules and their crate deps; embedders may use `default-features = false`.
...
- `ARCHIVE_VERSION` is 30; bump on incompatible bytecode, tag, or opcode changes. Approach A packed LA (`dot` / `matmul` / `Matrix` `*` `+` `-` / unary `-`) lowers to existing `HostInvoke` natives (`packed_dot` / `packed_matmul` / `packed_matrix_zip` / `packed_matrix_neg` in `machine/src/packed_la.rs`) — **no new LA opcodes**. Primitive casts (`CastIntToFloat` … `CastBoolToInt`) are appended after `TailCall`; `CastBoolToInt` is currently the last `Instruction` variant (keep the release `promise!` ceiling and `instruction_from_u8_covers_last_appended_variant` in sync). `TailCall` remains the self tail-recursion opcode. Static slots use `LoadStatic`/`StoreStatic`; `static_slot_count` is stored in the archive envelope. `.hyc` archives also carry `source_files` + `debug_locs` (one `DebugLoc` per bytecode slot); peephole fusion must call `shrink_debug_locs` alongside `fuse_bytecode`. Codegen applies scalar const folding (`ConstEnv`), constant branch/loop elimination, loop unroll (≤8 trips, no `break`/`continue`), optional tiny direct-call inlining, and `TailCall` for eligible self-recursive returns.
...
- `examples/fib.hy` is the smoke Fibonacci (`fib(10)` → `55`); `examples/fib_bench.hy` (also `fib(10)` → `55`) is the primary performance / dispatch regression entry (kept separate from the smoke example for `poop` / `vm_bench.sh` / `perf_metrics`).
...
- Virtual `regex` needs **libpcre2** (`libpcre2-dev` / Arch `pcre2`).
  The `pcre2-sys` crate can also build PCRE2 from source if a C
  toolchain is available. `examples/regex_demo.hy` and machine
  `regex::tests` exercise the HostInvoke path.
