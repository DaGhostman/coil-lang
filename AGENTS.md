...
- Machine/compiler Cargo features `crypto` and `time` (both default-on) gate the virtual `crypto` / `time` modules and their crate deps; embedders may use `default-features = false`.
