...
- Tests: `cargo test --workspace --lib --tests --bins` (required gate; covers integration tests, skips Criterion benches). Feature compile-gates: `cargo check --workspace --lib --tests --bins --no-default-features --features <crypto|time|regex|tls>`. Tooling: `--features <dissect|debugger>` with full test. Leak smoke: `ulimit -v 65536 && cargo run -- test`. Soft CPU: `./scripts/poop_baseline.sh`.
