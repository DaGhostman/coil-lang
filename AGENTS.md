...
- Virtual `io`: opaque `Stream`/`IoError`; `stdin`/`stdout`/`stderr`/`open`/`read`/`write`/`close` plus sync adapters and `from_bytes`/`to_bytes`; nested `io::net::tcp` / `io::net::udp` / `io::net::tls` (client `enable`/`disable`, server `encrypt`/`decrypt` on a TCP `Stream`, feature `tls`). L0 is non-blocking; buffers are `[byte]`.
...
- **Peephole.** Fusion must call `shrink_debug_locs` alongside `fuse_bytecode`. Nested VM returns from fused `ConstReturnImm` / `BinReturn` / `LoadReturnSlot` must call `capture_nested_return`.
- **BlockBuilder.** Placeholder-tracking only (no local byte buffer); positions are absolute in `Compiler::bytecode`. Direct emitters (`Print`, nested control flow, etc.) must extend `self.bytecode` in source order — do not return a local `Vec` for the caller to append.
- **Match.** Threaded-code layout with per-outer-tag groups; multi-arm groups emit an inner `JUMP_IF_MATCH` test chain. `JumpIfMatch`: `operands[31:16]` = tag, `value[31:0]` = absolute target. Nested record patterns use `UnpackAt` (slot-based), not top-of-stack `Unpack`.
