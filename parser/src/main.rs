...
impl ToString for ParserMessage {
    fn to_string(&self) -> String {
        self.message.clone()
    }
}

struct ParserBuilder<'parser> {
...
impl<'parser> ParserBuilder<'parser> {
        ...
        recursive(|expr| {
            ...
            let int = text::int(10)
                ...
                .validate(|v: Result<f64, ParseFloatError>, e, emitter| match v {
                    Ok(value) => value,
                    Err(msg) => {
                        emitter.emit(Rich::custom(e.span(), ParserMessage::new(msg.to_string())));
            ...
            let unary = negate.or(not).or(positive).boxed();
            let binary = unary
                .clone()
                .foldl_with(
                    choice((
    ...
    + Clone
    + 'parser {
        self.expression().then_ignore(op!(';')).boxed()
    }
    ...
    + 'parser {
        recursive(|stmt| {
            ...
            let print = text::keyword("print")
                .padded()
                .ignore_then(self.expression_stmt())
                ...
                match &t.1.borrow() {
                    Expression::Block(body) => {
                        for (idx, (s, expr)) in body.iter().enumerate() {
                            if idx < (body.len() - 1)
                                && matches!(expr.borrow(), Expression::Return(_))
                            {
                                emitter.emit(Rich::custom(
                                    *s,
                                    ParserMessage::new("All code after this is unreachable"),
                                ));
                                break;
                            }
                        }
                    ...
                    _ => emitter.emit(Rich::custom(
                        t.0,
                        ParserMessage::new("Unexpected expression/statement encountered"),
                    )),
                }
...
fn main() {
    ...
    let builder = ParserBuilder::new().build();

    match builder.parse(&src).into_result() {
        ...
        Err(errs) => {
            errs.into_iter().for_each(|e: Rich<'_, char>| {
                Report::build(ReportKind::Error, (file.clone(), e.span().into_range()))
                    ...
                    .with_label(
                        Label::new((file.clone(), e.span().into_range())).with_color(Color::Red),
                    )
                    .finish()
