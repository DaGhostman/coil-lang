...
test("prelude math trigonometry") {
    ...
    assert(approx(sin(pi / 2.0), 1.0, 0.000000000001))?;
    assert(approx(cos(0.0), 1.0, 0.000000000001))?;
    assert(approx(tan(pi / 4.0), 1.0, 0.000000000001))?;
}

test("prelude math scalar functions") {
    assert(approx(sqrt(2.0), 1.4142135623730951, 0.000000000001))?;
    assert(floor(0.0 - 1.25) == 0.0 - 2.0)?;
    assert(ceil(0.0 - 1.25) == 0.0 - 1.0)?;
    assert(approx(exp(1.0), 2.718281828459045, 0.000000000001))?;
    assert(approx(ln(2.718281828459045), 1.0, 0.000000000001))?;
    assert(pow(2.0, 10.0) == 1024.0)?;
    assert(approx(pow(9.0, 0.5), 3.0, 0.000000000001))?;
}

test("nested prelude math host invokes") {
    assert(approx(sqrt(pow(3.0, 2.0) + pow(4.0, 2.0)), 5.0, 0.000000000001))?;
}
