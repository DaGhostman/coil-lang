...
fn perf_numeric_uses_bin_slot_imm_jmpf_for_loop() {
    let (bc, _, _) = compile("examples/perf/numeric.0s");
    assert!(
...
fn perf_operators_loop_uses_log_not_jmpf() {
    let (bc, _, _) = compile("examples/perf/operators_loop.0s");
    assert!(
...
fn perf_numeric_dispatch_regression() {
    let (bc, pool, statics) = compile("examples/perf/numeric.0s");
...
fn perf_match_sum_emits_jump_if_match() {
    let (bc, _, _) = compile("examples/perf/match_sum.0s");
    assert!(
...
fn perf_fib_dispatch_regression() {
    let (bc, pool, statics) = compile("examples/fib_bench.0s");
    let dispatches = run_dispatch(bc, pool, statics);
    assert!(
