...
#[cfg(feature = "jit")]
#[test]
fn jit_direct_binary_call_uses_native_after_threshold() {
    ...
    let code = vec![
        const_int(20),
        const_int(22),
        Byte::new(Instruction::CALL).with_call_packed(2, 8),
...
#[cfg(feature = "jit")]
#[test]
fn jit_direct_immediate_call_uses_native() {
    let mut vm = Machine::<8>::default();
    vm.enable_jit(JitConfig {
...
#[cfg(feature = "jit")]
#[test]
fn jit_array_length_uses_vm_heap_helper() {
    let mut vm = Machine::<8>::default();
    vm.enable_jit(JitConfig {
        function_threshold: 1,
        ..JitConfig::default()
...
#[cfg(feature = "jit")]
#[test]
fn jit_array_index_uses_vm_heap_helper() {
    let mut vm = Machine::<8>::default();
    vm.enable_jit(JitConfig {
        function_threshold: 1,
        ..JitConfig::default()
...
#[cfg(feature = "jit")]
#[test]
fn jit_string_length_uses_vm_heap_helper() {
    let mut vm = Machine::<8>::default();
    vm.enable_jit(JitConfig {
        function_threshold: 1,
        ..JitConfig::default()
    ...
    let code = vec![
        ...
        Byte::new(Instruction::CALL).with_call_packed(1, 4),
        Byte::new(Instruction::HALT),
    ...
    vm.run_with_pool(&code, &[], &["hello".to_string()], 0);
...
#[cfg(feature = "jit")]
#[test]
fn jit_recursive_fib_call_uses_native_recursion() {
...
fn arity0_make_enum_reuses_immortal_singleton() {
    ...
    assert!(after > before, "expected one immortal enum allocation");
    let after_more = {
...
fn bin_slot_slot_powf_computes_float_power() {
    ...
    vm.run_with_pool(
        &[
            ...
            Byte::new(Instruction::BinSlotSlot).with_bin_slot_slot(Instruction::PowF as u8, 0, 1),
            Byte::new(Instruction::HALT),
        ],
        &pool,
        &[],
...
fn install_program(vm: &mut Machine<512>, code: &[Byte]) {
    vm.program_code =
        unsafe { std::slice::from_raw_parts(code.as_ptr().cast::<RawByte>(), code.len()).to_vec() };
...
fn debug_breakpoint_stops_before_target() {
    ...
    let code: &[Byte] = unsafe { std::slice::from_raw_parts(owned.as_ptr().cast(), owned.len()) };
...
fn with_operand_capacity_clamps_and_reports() {
    ...
    assert_eq!(
        huge.operand_stack_capacity(),
        crate::MAX_OPERAND_STACK_SLOTS
    );
